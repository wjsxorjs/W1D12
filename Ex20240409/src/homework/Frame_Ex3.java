...
public class Frame_Ex3 extends JFrame {
	...
	// 폭발 객체를 저장하는 곳
	ArrayList<Explosion_Ex3> ex_list = new ArrayList<>();
	
	// 폭발이미지
	Image[] exp_ar = new Image[27];
	
	// 운석을 저장할 수 있는 곳
	ArrayList<Meteor_Ex3> m_list = new ArrayList<>();
	...
	Thread makeMeteor = new Thread() {
		...
		public void run() {
			...
			while(flag) {
				//운석 생성
				Meteor_Ex3 m = new Meteor_Ex3(
					Frame_Ex3.this, 35, 30);
	...
	public Frame_Ex3() {
		...
		for(int i=0; i<exp_ar.length; i++) {
			// src/images/exp_enemy_1/exp_"1".png --> exp_ar[0]  i+1, i
			// src/images/exp_enemy_1/exp_"2".png
			// src/images/exp_enemy_1/exp_"3".png
